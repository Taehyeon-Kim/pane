...
fn get_tips_file_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("skills/claude-tips/data/claude-tips.yaml")
}
...
fn load_tips_from_yaml() -> Vec<serde_yaml::Value> {
    ...
    serde_yaml::from_str(&content)
        .unwrap_or_else(|e| panic!("Failed to parse tips YAML: {}", e))
}
...
fn test_claude_tips_search_filters_correctly() {
    ...
    let categories = ["prompting", "cost", "workflow", "debugging", "best-practices"];
